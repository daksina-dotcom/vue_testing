...
const { vCase, vFocus } = customDirect();
  ...
  if (validate()) {
    ...
    if (success) {
      try {
        ...
        setTimeout(() => {
          alert(`Welcome ${userEmail}. Your Id is:${Id}`);
          router.push("/table");
        },500);
      } catch (e) {
        console.error("Error decoding the token",e.message);
        router.push("/table");
      }
    } else {
...
<template>
  <v-toolbar :color="themeStore.header">
    <v-toolbar-title :style="{color: themeStore.font }"></v-toolbar-title>
    <v-switch
      v-model="isDark"
  ...
  <div class="total">
    <div class="form">
      ...
      <v-btn @click="addData">Login</v-btn>
    </div>
